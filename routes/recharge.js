...
router.get('/',function(req,res,next){
  ...
  sqlUtil.getResult(sql,function(result){
    ...
    console.log(sql);
    sqlUtil.getResult(sql,function(result){
      res.render('recharge',{title:'充值记录列表',result:result,pages:pages,page:offset/limit+1,begin_time:begin_time,end_time:end_time,phone:phone})
    })
  })
})
...
router.get('/execl',function(req,res,next){
  ...
  sqlUtil.getResult(sql,function(result){
    //构造execl数据
    var data=new Array();
    data[0]=['序号','手机号','充值金额','充值时间']
    for(var  i=0;i<result.length;i++){
      data[i+1]=new Array(result[i].id,result[i].phone,result[i].mount,result[i].create_time.toLocaleString());
    }
    var name="充值记录表";
    console.log(data);
    // data = [[1,2,3],[true, false, null, 'sheetjs'],['foo','bar',new Date('2014-02-19T14:30Z'), '0.3'], ['baz', null, 'qux']];
    var filename=encodeURI(new Date().toLocaleString()+'充值记录.xlsx');
...
router.post('/new',function(req,res,next){
  var mount=parseInt(req.body.mount);
  var balance=parseInt(req.body.balance)+mount;
  var phone=req.body.phone;
  var sql='update vip set mount='+balance+' where phone='+phone;
  console.log(sql);
  sqlUtil.getResult(sql,function(result){
    var create_time=new Date().toLocaleString();
    sql='insert into recharge(phone,mount,create_time) values("'+phone+'",'+mount+',"'+create_time+'")';
    sqlUtil.getResult(sql,function(result){
      res.redirect('/vip');
    })
  })
